...
\keys{\cmd+L}, \keys{J} Forward search from tex to pdf \\
\keys{\cmd+L}, \keys{T}, \keys{F} Toggle window focus to pdf \\
\keys{\cmd+L}, \keys{T}, \keys{S} Toggle pdf syncing after build \\
\keys{\cmd+L}, \keys{T}, \keys{?} Status of toggles \\
\keys{\cmd+L}, \keys{\backspace} Remove temp files \\
\keys{\cmd+R} Goto sections and labels \\
\keys{\cmd+L}, \keys{C} Turn word into command \\
\keys{\cmd+L}, \keys{E} Turn word into environment \\
...
\keys{\cmd+L}, \keys{\cmd+E} Emphasis/italics \\
\keys{\cmd+L}, \keys{\cmd+B} Bold \\
\keys{\cmd+L}, \keys{\cmd+U} Underlined \\
\keys{\cmd+L}, \keys{\cmd+T} Typewriter  \\
\keys{\ctrl+\space} Auto-complete for everything \\
% \textbackslash{}ref, \keys{\cmd+L}, \keys{\ctrl+\space} Keyword to be followed by autocomplete to get references to labels \\
...
\subsection{\LaTeX{}ing}

\subsection{Markdown}
\textbf{remapped} \keys{\Alt+M} Preview Markdown in Browser\\
...
\keys{\shift+\cmd+Y} Evaluate math expression
