...
\keys{\cmd+L}, \keys{\backspace} Remove temp files (remap) \\
\keys{\cmd+R} Goto sections and labels \\
\keys{\cmd+L}, \keys{\cmd+S} Show snippets \\
...
\keys{\cmd+L}, \keys{\shift+\cmd+\E} Rename environment \\
\keys{\shift+\cmd+8} Toggle star environment \\
\keys{\cmd+L}, \keys{\cmd+W}, \keys{E} Emphasis/italics \\
\keys{\cmd+L}, \keys{\cmd+W}, \keys{B} Bold \\
\keys{\cmd+L}, \keys{\cmd+W}, \keys{U} Underlined \\
\keys{\cmd+L}, \keys{\cmd+W}, \keys{T} Typewriter  (remap)\\
...
\subsection{Markdown}
\keys{\Alt+M} Preview Markdown in Browser (remap)\\
...
\subsection{Emmet}
\keys{\ctrl+X} Expand abbreviation (e.g. \texttt{ul>li.listitem*6>a}) (remap)\\
